...
namespace SmartStore.LMS
{
    public class ModelBoundEventConsumer : IConsumer
    {
        ...
        {
            if (!eventMessage.BoundModel.CustomProperties.ContainsKey("LMS"))
                return;

            var model = eventMessage.BoundModel.CustomProperties["LMS"] as AdminEditTabModel;
            if (model == null)
                return;

            var settings = _settingService.LoadSetting<LMSSettings>(_storeContext.CurrentStore.Id);
